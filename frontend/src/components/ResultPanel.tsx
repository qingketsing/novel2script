...
export function ResultPanel({ copyState, result, onCopy, onDownload, onOpenPreview }: ResultPanelProps) {
  ...
  return (
    <section className="flex min-h-[620px] flex-col gap-4 rounded-lg border border-zinc-200 bg-white p-4 shadow-sm shadow-zinc-200/60">
      <div className="flex flex-col justify-between gap-3 border-b border-zinc-200 pb-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            {result ? `已识别 ${result.chapter_count} 个章节，生成模式：${generationModeLabel(result.mode)}` : "生成后显示结构化结果"}
          </p>
        </div>
